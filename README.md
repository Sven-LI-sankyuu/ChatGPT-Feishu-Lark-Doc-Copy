...
这是一个 Chrome 和 Edge 浏览器插件。它在 ChatGPT 官方复制按钮旁增加一个小按钮，点击后取得该回复的官方 Markdown，再转换为适合粘贴到飞书文档的版本。官方复制按钮本身的点击行为保持原样。
...
插件只在点击飞书按钮时读取一次剪贴板，用于取得 ChatGPT 官方复制结果并立即写回转换后的文本；它不会在后台监听剪贴板。
...
所有转换都在当前页面本地完成，不上传聊天内容，不加载远程代码。插件申请剪贴板读写权限，只用于用户主动点击飞书按钮后的单次转换。
